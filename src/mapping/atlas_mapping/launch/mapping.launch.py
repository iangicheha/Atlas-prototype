"""
atlas_mapping/launch/mapping.launch.py

SLAM Toolbox online-async mapping session for Atlas.
...
Prerequisites (must be running before this launch file):
  - Gazebo + robot_state_publisher          (atlas_gazebo/gazebo.launch.py)
  - ros2_control + velocity_smoother        (atlas_control/control.launch.py)
  - EKF (robot_localization ekf_node)       (atlas_localization/localization.launch.py)
...
Usage:
  ros2 launch atlas_mapping mapping.launch.py
  ros2 launch atlas_mapping mapping.launch.py rviz_enabled:=false
  ros2 launch atlas_mapping mapping.launch.py slam_params_file:=/path/to/custom.yaml
"""
...
def generate_launch_description():
    pkg = FindPackageShare('atlas_mapping')
