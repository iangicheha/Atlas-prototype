"""nav_client.py — Programmatic NavigateToPose action client for atlas_navigation.

Usage examples
--------------
# Send a single goal:
ros2 run atlas_navigation nav_client

# Override goal coordinates via CLI args:
ros2 run atlas_navigation nav_client --ros-args -p x:=3.0 -p y:=2.0 -p yaw:=0.0

# Send a patrol (series of waypoints) using the FollowWaypoints action:
ros2 run atlas_navigation nav_client --ros-args -p mode:=patrol
