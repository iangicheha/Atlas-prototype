"""
atlas_bringup/launch/simulation.launch.py

Top-level simulation entrypoint — delegates to the simulator-specific launch file.

Supported backend:
  gazebo  — atlas_gazebo/launch/gazebo.launch.py
...
Usage:
  ...
  ros2 launch atlas_bringup simulation.launch.py simulator:=gazebo world:=empty
  ros2 launch atlas_bringup simulation.launch.py mapping_enabled:=true
  ros2 launch atlas_bringup simulation.launch.py use_amcl:=true map_yaml_file:=/path/to/map.yaml
...
def generate_launch_description():
    ...
    gazebo_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [FindPackageShare("atlas_gazebo"), "launch", "gazebo.launch.py"]
            )
        ),
    ...
    teleop_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [FindPackageShare("atlas_teleop"), "launch", "teleop.launch.py"]
            )
        ),
    ...
    localization_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [FindPackageShare("atlas_localization"), "launch", "localization.launch.py"]
            )
        ),
        launch_arguments={
            "use_sim_time": "true",
    ...
    camera_processing_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [
                    FindPackageShare("atlas_camera_processing"),
                    "launch",
    ...
    mapping_bringup = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution(
                [FindPackageShare("atlas_mapping"), "launch", "mapping.launch.py"]
            )
        ),
        launch_arguments={
            "use_sim_time":  "true",
