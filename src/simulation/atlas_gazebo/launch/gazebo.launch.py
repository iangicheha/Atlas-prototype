"""
atlas_gazebo/launch/gazebo.launch.py
...
What this launches:
  ...
  3. ros_gz_sim/create  — spawns atlas_bot into the running world (TimerAction 3 s delay)
  4. ros_gz_bridge/parameter_bridge  — bridges topics per ros_gz_bridge.yaml

Usage:
  ros2 launch atlas_gazebo gazebo.launch.py
  ros2 launch atlas_gazebo gazebo.launch.py world:=empty x:=1.0 y:=2.0
"""
...
def _resolve_world_path(context):
    ...
    return os.path.join(
        get_package_share_directory("atlas_gazebo"),
        "worlds",
        f"{world}.sdf",
    )
...
def _gazebo_resource_path(context):
    gazebo_share = get_package_share_directory("atlas_gazebo")
    bundled_paths = [
        os.path.join(gazebo_share, "models"),
        gazebo_share,
        get_package_share_directory("atlas_meshes"),
    ]
    existing_paths = _path_entries(os.environ.get("GZ_SIM_RESOURCE_PATH", ""))
    extra_paths = _path_entries(
        LaunchConfiguration("extra_gz_resource_path").perform(context)
...
def generate_launch_description():
    pkg_gz = FindPackageShare("atlas_gazebo")
    pkg_desc = FindPackageShare("atlas_description")
    pkg_control = FindPackageShare("atlas_control")
    ...
    declared_args = [
        DeclareLaunchArgument(
            ...
            description="World name (must match a .sdf file in atlas_gazebo/worlds/)",
        ),
